...
import TaskDetailDrawer from '../../components/portal/TaskDetailDrawer'; // <--- 1. IMPORT THE DRAWER
import { FaPlus, FaTrash, FaCheckCircle, FaHourglassHalf, FaBriefcase, FaClipboardList, FaTag, FaUserShield, FaUserClock } from 'react-icons/fa';
import { toast } from 'react-toastify';
import '../../styles/portal.css';
...
const EmployeeDashboard = () => {
  const { user } = useContext(AuthContext);
  const [tasks, setTasks] = useState([]);
  const [activeTab, setActiveTab] = useState('assigned'); // 'personal' or 'assigned'
...
useEffect(() => {
    if (tasks.length > 0) {
      ...
      const lastWeekTasks = tasks.filter(t => !t.isPersonal && t.status === 'Completed');
      
      // Create data for the chart (Showing top 4 most time-consuming completed tasks)
      const chartData = lastWeekTasks
        .map(t => ({ name: t.title.substring(0, 10) + '...', hours: t.timeLogged || 0 }))
  ...
  const handleAddNote = async (taskId, noteText) => {
    try {
      const res = await api.post(`/tasks/${taskId}/notes`, { text: noteText });
      
      // Update local state instantly so the drawer UI refreshes without closing
      setSelectedTask(res.data); 
      
      // Refresh background tasks quietly
      fetchTasks(); 
  ...
  <h1 className="ept-title" style={{textAlign: 'left', marginBottom: '10px'}}>My Workspace</h1>
      ...
      {/* --- PROFESSIONAL EMPLOYEE METRICS --- */}
      <div className="lexa-anl-grid" style={{ marginBottom: '30px', gridTemplateColumns: '1fr 1fr' }}>
        
        {/* Left Side: Stats */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div className="ept-card" style={{ borderLeft: '4px solid #06B6D4' }}>
            <h3><FaUserShield /> Assigned Work</h3>
            <div className="value">{assignedTasks.length}</div>
            <p style={{fontSize: '0.8rem', color: '#94A3B8'}}>Total projects given</p>
          </div>
          <div className="ept-card" style={{ borderLeft: '4px solid #EAB308' }}>
            <h3><FaHourglassHalf /> Pending</h3>
            <div className="value" style={{ color: '#EAB308' }}>{tasks.filter(t => t.status !== 'Completed').length}</div>
            <p style={{fontSize: '0.8rem', color: '#94A3B8'}}>Requires attention</p>
          </div>
          <div className="ept-card" style={{ borderLeft: '4px solid #10B981', gridColumn: 'span 2' }}>
            <h3><FaCheckCircle /> Total Hours Logged</h3>
            <div className="value" style={{ color: '#10B981' }}>
              {tasks.reduce((sum, t) => sum + (t.timeLogged || 0), 0)} Hrs
            </div>
            <p style={{fontSize: '0.8rem', color: '#94A3B8'}}>Across all tasks</p>
          </div>
        </div>

        {/* Right Side: Productivity Chart */}
        <div className="lexa-anl-card">
          <h3 className="lexa-anl-title">Time Distribution (Top Completed Tasks)</h3>
            ...
            {performanceData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={performanceData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                  ...
                  <YAxis stroke="#94A3B8" />
                  <Tooltip cursor={{fill: 'rgba(255,255,255,0.05)'}} contentStyle={{backgroundColor: '#020617', borderColor: '#06B6D4', borderRadius: '8px'}} />
                  <Bar dataKey="hours" fill="#06B6D4" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <div style={{display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center', color: '#64748B'}}>
                Complete tasks and log hours to see analytics.
              </div>
            )}
          </div>
        </div>
       ...
       {activeTab === 'assigned' && (
          <div style={{ paddingTop: '10px' }}>
            {assignedTasks.length === 0 ? (
              <p style={{color: '#64748B', textAlign: 'center', padding: '40px'}}>No work assigned from Admin yet.</p>
            ) : (
              <KanbanBoard 
                tasks={assignedTasks} 
                onStatusChange={handleStatusUpdate} 
                onTaskClick={handleTaskClick} // <--- 4. PASS CLICK HANDLER
              />
            )}
          </div>
        )}
      ...
      <TaskDetailDrawer 
        ...
        currentUser={{ id: user._id || user.id, name: user.name }}
      />
    </PortalLayout>
  );
};
